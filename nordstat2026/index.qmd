...
[Julia packages (unoptimized)]{.alertblue}
...
$$
\begin{aligned}
y_t \vert \v x_{t}, z_{t} &\sim \mathrm{Beta}\big(\mu_t = \mathrm{logistic}(\v x_{t}^\top\v\beta_{t}), \psi_t = \exp(\v z_{t}^\top\v\gamma_{t}) \big) 
\end{aligned}
$$
</div>
...
:::: {.columns}
::: {.column width="70%"}
- [Forward-filtering-backward sampling]{.alertred} (FFBS) for linear Gaussian models [@carter1994gibbs; @fruhwirth1994data]
  - Run Kalman filter $p(\v\theta_t\vert y_{1:t})$ forward for $t=1,2,\ldots,T$
  - Sample $p(\v\theta_{t}\vert \v\theta_{t+1}, y_{1:T})$ backward for $t=T-1,T-2,\ldots,1$
...
## Grouping for speed and Gaussianity

- Need Gaussian approximations at every time step $t$

- The prior $\v\theta_t \sim N(\v\theta_{t\vert t-1}, \Omega_{t \vert t-1} )$ helps a lot.

- No Bernstein-von Mises effect to make the likelihood contribution at time $t$ more Gaussian.

- [Group observations]{.alertred}: $\v y_g = (y_t,y_{t-1},\ldots,y_{t-G+1})^\top$ share the *same* state/parameter $\v\theta_g$:
  - Subsampling MCMC/HMC [@salomone2020spectral]
  - Filtering [Särkää]
  - Macroeconomic modeling [@celanimoderatetime]
  
- Two views: 
  - interpolation for algorithmic performance
  - genuine model change: different time scales for observations and parameters.

## Natural scaled parameter evolution
