---
title: " Bayesian inference in non-Gaussian time series with global-local shrinkage process priors"
...
-  [Oskar Gustafsson]{.alertred}, Konjunkturinstitutet and Stockholm University
...
[mattiasvillani.com/seminars/vaxjo2026](mattiasvillani.com/seminars/vaxjo2026)
:::
...
3.  Bayesian inference for Non-Gaussian models
4.  Grouping observations
5.  Natural parameter innovations
...
-   [DSP priors]{.alertred} on $\v\beta_t$.
...
- [Gaussian approximations]{.alertred}
...
- Non-linear/non-Gaussian: replace the filtering step with an [approximate Gaussian filter]{.alertred}.
...
- Nonlinear Gaussian models
  ...
  - [Iterated extended/unscented Kalman filter]{.alertred} [@skoglund2015extended, @skoglund2019iterative]
...
- Non-Gaussian models
  ...
  - [Posterior linearization filter - sufficient statistics]{.alertred}. 
...
## Beta regression for US layoff proportion 

![](figs/layoff_group5.png){width="80%" fig-align="center"}
...
- [Group/segment observations]{.alertred} in $G$ groups of $J$ observations. 
- [Same state $\v\beta_g$ for observations within a group]{.alertred}.

$$
\begin{aligned}
y_{t} \vert \v x_{t} &\sim p(y_t \vert \v x_t, \v\beta_{g(t)}) \\
...
\end{aligned}
$$
where $g(t) = \lceil t/J \rceil$ maps time periods to segments/groups.

![](figs/groups.png){width="60%" fig-align="center"}
...
## Beta regression for US layoff proportion - groups of 2, 5 and 7

![](figs/layoff5.svg){width="80%" fig-align="left"}
...
- Parameter evolution should adapt to:
  - Scales and covariance of covariates
  - Link functions
  - Model geometry
  
- Information geometry ideas [@amari1998natural; @amari1982differential]

- Observation driven models (Generalized autoregressive score models) [@creal2013generalized]
  
- [Natural parameter innovations]{.alertred}  

$$
\v\beta_{t} = \v\beta_{t-1} + \mathcal{I}(\v\beta_{t-1})^{-1/2}\v\nu_t
$$

- Controls the KL divergence over time 

$$
D_{\mathrm{KL}}\big( p(y \vert \v\beta) \, \vert \vert \, p(y \vert \v\beta +\v\epsilon) \big) \approx  \frac{1}{2}\v\epsilon^\top \mathcal{I}(\v\beta)\v\epsilon,
$$
:::

::: {.column width="50%"}

![](figs/KLPoissonLambdaNoScaling.svg){width="100%" fig-align="center"}

![](figs/KLPoissonLambdaScaling.svg){width="100%" fig-align="center"}
:::
...
- Non-linear transition model. $\mathcal{I}(\v\beta_{t-1}) \approx \mathcal{I}(\hat{\v\beta}_{t-1 \vert t-1})$.
- Fisher information from one observation is [unit rank]{.alertred}. 
- Poisson regression: $$\mathcal{I}(\v\beta) = \lambda_t \v x_t \v x_t^\top $$
where $\lambda_t = \exp(\v x_t ^\top \v\beta)$.
- Many ways around this:

  - Pseudo inverse [@creal2013generalized]
  - Grouping observations
  - Average of all observations $$\bar{\mathcal{I}}(\v\beta) = \frac{1}{T}\v X^\top \mathrm{Diag}(\lambda_1,\ldots,\lambda_T) \v X$$
  
  

## US layoff with different scaling of innovations

- Left to right: no scaling, global full, local full, global diagonal, local diagonal.

![](figs/layoff_diff_s.svg){width="100%" fig-align="left"}

## Natural parameter innovations - Poisson regression simulation study

![](figs/PoisSimScaling.svg){width="90%" fig-align="center"}




## Seasonal Poisson regression for bike rides

![](figs/bikefullscaling.png){width="100%" fig-align="left"}

## Ongoing and future work

- [Multivariate]{.alertred} extensions, including mixed data types.
